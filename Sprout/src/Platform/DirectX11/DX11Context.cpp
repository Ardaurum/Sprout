...
namespace Sprout
{
	...
	{
		SPROUT_CORE_ASSERT(!Instance, "DX11 Context already exists!");
		Instance = this;
	}
	...
	void DX11Context::Init()
	{
		...
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to create D3D11 Device!");
		...
		swapChainDesc.OutputWindow = glfwGetWin32Window(WindowHandle);
		...
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to get dxgiDevice!");

		Microsoft::WRL::ComPtr<IDXGIAdapter> dxgiAdapter = 0;
		result = dxgiDevice->GetParent(__uuidof(IDXGIAdapter), (void**) &dxgiAdapter);
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to get dxgiAdapter!");
		...
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to get dxgiFactory!");

		result = dxgiFactory->CreateSwapChain(Device.Get(), &swapChainDesc, &SwapChain);
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to create SwapChain!");
	...
	{
		SPROUT_CORE_ASSERT(Device && Context && SwapChain, "Initialize context first!");

		int width, height;
		glfwGetWindowSize(WindowHandle, &width, &height);

		HRESULT result = SwapChain->ResizeBuffers(2, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to resize swap chain buffers!");

		Microsoft::WRL::ComPtr<ID3D11Texture2D> backBuffer = 0;
		result = SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**) &backBuffer);
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to get buffer from swap chain!");

		result = Device->CreateRenderTargetView(backBuffer.Get(), 0, &RenderTarget);
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to create Render Target View!");
		...
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to create depth stencil texture!");

		result = Device->CreateDepthStencilView(DepthStencil.Get(), 0, &DepthStencilView);
		SPROUT_CORE_ASSERT(SUCCEEDED(result), "Failed to create depth stencil view!");
